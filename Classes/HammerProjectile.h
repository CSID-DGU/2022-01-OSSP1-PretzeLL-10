#ifndef __HAMMER_PROJECTILE_H__
#define __HAMMER_PROJECTILE_H__

#include "BaseBullet.h"

class HammerProjectile : public BaseBullet {
protected:
	cocos2d::Node* __hero;
	cocos2d::Vec2 __initial_pos;
	float __desired_distance = 500.0f;

	HammerProjectile() : BaseBullet("weapon_hammer") {}
	virtual ~HammerProjectile() {}

public:
	CREATE_FUNC(HammerProjectile);
...
//            move();                                                             // move to apply force
			...
			schedule(schedule_selector(HammerProjectile::followTarget));         // schedule follow function
		}
	}
	...
	void followTarget(float dt) {
		...
		if (len < 20.0f) {
			unschedule(schedule_selector(HammerProjectile::followTarget));           // if length is close enough, remove this
			removeAfter(0.0f);
		}
	}
};
