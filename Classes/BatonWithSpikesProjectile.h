#ifndef __BATON_WITH_SPIKES_PROJECTILE_H__
#define __BATON_WITH_SPIKES_PROJECTILE_H__

#include "BaseBullet.h"

class BatonWithSpikesProjectile : public BaseBullet {
protected:
	cocos2d::Node* __hero;
	cocos2d::Vec2 __initial_pos;
	float __desired_distance = 500.0f;

	BatonWithSpikesProjectile() : BaseBullet("weapon_baton_with_spikes") {}
	virtual ~BatonWithSpikesProjectile() {}

public:
	CREATE_FUNC(BatonWithSpikesProjectile);
...
//            move();                                                             // move to apply force
			...
			schedule(schedule_selector(BatonWithSpikesProjectile::followTarget));         // schedule follow function
		}
	}
	...
	void followTarget(float dt) {
		...
		if (len < 20.0f) {
			unschedule(schedule_selector(BatonWithSpikesProjectile::followTarget));           // if length is close enough, remove this
			removeAfter(0.0f);
		}
	}
};

#endif /* __BATON_WITH_SPIKES_PROJECTILE_H__ */