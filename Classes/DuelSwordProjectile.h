#ifndef __DUEL_SWORD_PROJECTILE_H__
#define __DUEL_SWORD_PROJECTILE_H__

#include "BaseBullet.h"

class DuelSwordProjectile : public BaseBullet {
protected:
	cocos2d::Node* __hero;
	cocos2d::Vec2 __initial_pos;
	float __desired_distance = 500.0f;

	DuelSwordProjectile() : BaseBullet("weapon_duel_sword") {}
	virtual ~DuelSwordProjectile() {}

public:
	CREATE_FUNC(DuelSwordProjectile);
...
//            move();                                                             // move to apply force
			...
			schedule(schedule_selector(DuelSwordProjectile::followTarget));         // schedule follow function
		}
	}
	...
	void followTarget(float dt) {
		...
		if (len < 20.0f) {
			unschedule(schedule_selector(DuelSwordProjectile::followTarget));           // if length is close enough, remove this
			removeAfter(0.0f);
		}
	}
};

#endif /* __DUEL_SWORD_PROJECTILE_H__ */