...
Hero::Hero()
    : DynamicObject("knight_f"), __weapon(std::make_pair(std::vector<weapon_t *>(3), 0)), __map_dir(MAP_NONE), __invincible(false), __disarmed(false)
{
...
void Hero::disarm(float time)
{
    ...
    __disarmed = true;
    if (time)
        scheduleOnce(schedule_selector(Hero::rearm), time);
}
...
DIRECTION Hero::getDirection(bool isAbleToMove)
{
    ...
    {
    case MAP_UP:
        setAbsolutePosition(525.0f, 375.0f);
        break;
    case MAP_DOWN:
        setAbsolutePosition(525.0f, 850.0f);
        break;
    case MAP_LEFT:
        setAbsolutePosition(850.0f, 650.0f);
        break;
    case MAP_RIGHT:
        setAbsolutePosition(150.0f, 650.0f);
        break;
    default:
        break;
    }
