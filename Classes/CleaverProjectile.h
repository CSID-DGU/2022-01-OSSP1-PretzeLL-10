#ifndef __CLEAVER_H_PROJECTILE_H__
#define __CLEAVER_H_PROJECTILE_H__

#include "BaseBullet.h"

class CleaverProjectile : public BaseBullet {
protected:
	cocos2d::Node* __hero;
	cocos2d::Vec2 __initial_pos;
	float __desired_distance = 500.0f;

	CleaverProjectile() : BaseBullet("weapon_cleaver") {}
	virtual ~CleaverProjectile() {}

public:
	CREATE_FUNC(CleaverProjectile);
...
//            move();                                                             // move to apply force
			...
			schedule(schedule_selector(CleaverProjectile::followTarget));         // schedule follow function
		}
	}
	...
	void followTarget(float dt) {
		...
		if (len < 20.0f) {
			unschedule(schedule_selector(CleaverProjectile::followTarget));           // if length is close enough, remove this
			removeAfter(0.0f);
		}
	}
};

#endif /* __CLEAVER_H_PROJECTILE_H__ */